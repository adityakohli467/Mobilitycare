...
class ControllerInformationDemoRequest extends Controller {
	...
	public function index() {
	  ...
	  if ($this->request->server['REQUEST_METHOD'] == 'POST' && ($skip_validation || $this->validate())) {
                ...
                try {
                    ...
                    $this->log->write('DEMO REQUEST DB SAVE ERROR: ' . $dbError->getMessage());
                }
    ...
    $mail->send();
        ...
        $this->log->write('DEMO REQUEST ADMIN MAIL ERROR: ' . $e->getMessage());
        try {
            ...
            $fallback->setSubject('[ALERT] Demo Request Received - Email Delivery Issue');
            $fallback->setHtml('<p>A demo request was saved to the database but the full notification email failed.</p><p><b>Error:</b> ' . htmlspecialchars($e->getMessage()) . '</p><p>Please check admin panel.</p>' . $mailMessageHtml);
            $fallback->send();
        } catch (\Throwable $e2) {
            $this->log->write('DEMO REQUEST FALLBACK MAIL ERROR: ' . $e2->getMessage());
        }
    }
    ...
    try {
        ...
        $this->log->write('DEMO REQUEST CUSTOMER MAIL ERROR: ' . $e->getMessage());
    }
