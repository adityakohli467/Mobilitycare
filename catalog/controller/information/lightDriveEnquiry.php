...
class ControllerInformationLightDriveEnquiry extends Controller {
    ...
    try {
        ...
        $fallback->setSubject('[ALERT] Light Drive Enquiry Received - Email Delivery Issue');
        $fallback->setHtml('<p>A Light Drive enquiry was saved to the database but the full notification email failed.</p><p><b>Error:</b> ' . htmlspecialchars($e->getMessage()) . '</p><p>Please check admin panel.</p>' . $mailMessageHtml);
        $fallback->send();
    } catch (\Throwable $e2) {
        $this->log->write('LIGHTDRIVE FALLBACK MAIL ERROR: ' . $e2->getMessage());
    }
}
